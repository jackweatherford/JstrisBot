...
def on_press(key):
  if key == Key.space or key == Key.down or repr(key) == "'c'":
    ...
    try:
      update(piece_color)
    except FailSafeException: # PyAutoGUI fail-safe (mouse moving to a corner of the screen)
      exit('Mouse moved to a corner of the screen - activating PyAutoGUI fail-safe - Terminating')
